...
class GANJI2(threading.Thread):
    ...
    def crawler_level2(self,SearchUrl):
        ...
        try:
            ...
            area = ''
            EstateName = ''
            for ele in sourcebsObj.findAll("li"):
                try:
                    prop = re.sub('[\t\r\n ]','',ele.text)
                except:
                   continue
                if "户型" in prop:
                    try:
                        roomprops1 = ele.text.split('：')[1].strip().split('-')
                    ...
                    try:
                        floor = re.findall(unicode('：([\W|\w]+)\（'),prop)[0]
                    except:
                        floor = unicode('其他')
                    floorAll = re.findall(unicode('共(\d+)层'),prop)[0]
                elif "位置" in prop:
                    position = prop.split('：')[1].split('-')
                    district = position[1]
                    if len(position) == 2:
                        area = position[1]
                    else:
